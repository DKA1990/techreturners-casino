...
import { GameState, isSuccessResponse, Card } from "../types/game-types";
import AppButton from "./AppButton";

export const StartGameControl: React.FC = () => {
  const { stateOfGame, setStateOfGame, cards, setCards } = useGame();

  const { data, isFetching, error, status, execute } = useFetchData(
  ...
  return <AppButton onClick={handleClick}>Start Game</AppButton>;
};
